<?php
	...
	$social_icon_class = 'cryptro-style-' . cryptro_get_option('general', 'top-bar-social-style', 'round');

	foreach( $social_list as $social_key => $social_icon ){
		$social_link = cryptro_get_option('general', 'top-bar-social-' . $social_key);
		...
		if( !empty($social_link) ){
			echo '<a href="' . esc_attr($social_link) . '" target="_blank" class="cryptro-top-bar-social-icon ' . esc_attr($social_icon_class) . '" title="' . esc_attr($social_key) . '" >';
