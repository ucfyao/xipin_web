<?php
	...
	$title_style = cryptro_get_option('general', 'default-title-style', 'small');
	$title_align = cryptro_get_option('general', 'default-title-align', 'left');

	// for index page
	if( is_home() ){
		$main_head = true;
		$page_title = esc_html__('Blog', 'cryptro');
	...
	// for pages
		...
		if( !empty($post_option['enable-breadcrumbs']) ){
	...
	// search page
		...
		if( have_posts() ){
			$page_title = esc_html__('Search Results', 'cryptro');
	...
	// archive page
		...
		if( is_category() || is_tax('portfolio_category') || is_tax('product_cat') ){
			$page_title = esc_html__('Category', 'cryptro');
			...
			$page_title = esc_html__('Tag', 'cryptro');
			...
			$page_title = esc_html__('Day','cryptro');
			$page_caption = get_the_date('F j, Y');
		}else if( is_month() ){
			$page_title = esc_html__('Month','cryptro');
			$page_caption = get_the_date('F Y');
		}else if( is_year() ){
			$page_title = esc_html__('Year','cryptro');
			$page_caption = get_the_date('Y');
		}else if( is_author() ){
			$page_title = esc_html__('By','cryptro');
			
			$author_id = get_query_var('author');
			$author = get_user_by('id', $author_id);
			$page_caption = $author->display_name;					
		}else if( is_post_type_archive('product') ){
			$page_title = esc_html__('Shop', 'cryptro');
			$page_caption = '';
		}else{
			$page_title = get_the_title();
			$page_caption = '';
		...
		if( $title_style == 'plain' && !empty($page_caption) ){
	...
	if( (empty($post_option['enable-page-title']) || $post_option['enable-page-title'] == 'enable') && (!empty($page_title) || !empty($page_caption)) ){	
		...
		$page_title = apply_filters('cryptro_page_title', $page_title);
		$page_caption = apply_filters('cryptro_page_caption', $page_caption);

		$extra_class  = ' cryptro-style-' . $title_style;
		$extra_class .= ' cryptro-' . $title_align . '-align';

		echo '<div class="cryptro-page-title-wrap ' . esc_attr($extra_class) . '" ' . gdlr_core_esc_style(array(
			'background-image' => empty($title_background)? '': $title_background
		)) . '>';
		echo '<div class="cryptro-header-transparent-substitute" ></div>';
		echo '<div class="cryptro-page-title-overlay" ' . gdlr_core_esc_style(array(
		...
		echo '<div class="cryptro-page-title-container cryptro-container" >';
		echo '<div class="cryptro-page-title-content cryptro-item-mglr" ' . gdlr_core_esc_style(array(
		...
		if( !empty($page_title) ){
			if( !empty($main_head) ){
				echo '<h1 class="cryptro-page-title" ' . gdlr_core_esc_style(array(
				...
				echo '<h3 class="cryptro-page-title" ' . gdlr_core_esc_style(array(
					'font-size' => empty($title_size['title-size'])? '': $title_size['title-size'],
					'font-weight' => empty($title_weight['title-weight'])? '': $title_weight['title-weight'],
					'text-transform' => (empty($title_transform) || $title_transform == 'default')? '': $title_transform,
					'letter-spacing' => empty($title_size['title-letter-spacing'])? '': $title_size['title-letter-spacing'],
		...
		if( !empty($page_caption) ){
			echo '<div class="cryptro-page-caption" ' . gdlr_core_esc_style(array(
