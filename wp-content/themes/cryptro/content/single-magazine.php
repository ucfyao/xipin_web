<?php
	...
	echo '<div class="cryptro-single-magazine-author-tags clearfix" >';
	...
	if( cryptro_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		...
		if( !empty($prev_post) ){
			$prev_post_html = get_previous_post_link(
				...
				'<span class="cryptro-text cryptro-title-font" >' . esc_html__( 'Previous Post', 'cryptro' ) . '</span>' .
				'%link' . 
				'</span>',
		...
		if( !empty($next_post) ){
			$next_post_html = get_next_post_link( 
				...
				'<span class="cryptro-text cryptro-title-font" >' . esc_html__( 'Next Post', 'cryptro' ) . '</span>' . 
				'%link' . 
				'</span>',
				'<span class="cryptro-single-nav-title cryptro-title-font" >%title</span>'
			);
		...
		if( !empty($prev_post_html) || !empty($next_post_html) ){
			echo '<div class="cryptro-single-nav-area clearfix" >' . $prev_post_html . $next_post_html . '<div class="cryptro-single-nav-area-divider" ></div></div>';
		}
	}
	...
	if( $enable_related_post == 'enable' && class_exists('gdlr_core_blog_style') ){
		...
		if( $query->have_posts() ){
			...
			echo '<div class="cryptro-single-related-post-wrap cryptro-item-rvpdlr" >';
			echo '<h3 class="cryptro-single-related-post-title cryptro-item-mglr" >' . esc_html__('Related Posts', 'cryptro') . '</h3>';
