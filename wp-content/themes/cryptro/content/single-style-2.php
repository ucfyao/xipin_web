<?php
	...
	echo '<div class="cryptro-single-author-tags clearfix" >';
	...
	if( cryptro_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		...
		if( !empty($prev_post) ){
			$prev_post_html = get_previous_post_link(
				'<span class="cryptro-single-nav-area-left" >%link</span>',
				'<span class="cryptro-single-nav-area-background" ' . gdlr_core_esc_style(array('background-image' => get_post_thumbnail_id($prev_post->ID))) . ' ></span>' . 
				'<span class="cryptro-single-nav-area-overlay" ></span>' . 
				'<span class="cryptro-single-nav cryptro-single-nav-left">' . 
				'<i class="arrow_left" ></i>' . 
				'<span class="cryptro-text" >' . esc_html__( 'Previous Post', 'cryptro' ) . '</span>' .
				'<span class="cryptro-single-nav-title cryptro-title-font" >%title</span>' .
				'</span>'
			);
		}
		...
		if( !empty($next_post) ){
			$next_post_html = get_next_post_link(
				'<span class="cryptro-single-nav-area-right" >%link</span>',
				'<span class="cryptro-single-nav-area-background" ' . gdlr_core_esc_style(array('background-image' => get_post_thumbnail_id($next_post->ID))) . ' ></span>' .
				'<span class="cryptro-single-nav-area-overlay" ></span>' .
				'<span class="cryptro-single-nav cryptro-single-nav-right">' . 
				'<span class="cryptro-text" >' . esc_html__( 'Next Post', 'cryptro' ) . '</span>' . 
		...
		if( !empty($prev_post_html) || !empty($next_post_html) ){
			echo '<div class="cryptro-single-nav-area clearfix" >' . $prev_post_html . $next_post_html . '</div>';
		}
	}


	// related post
	$enable_related_post = cryptro_get_option('general', 'enable-related-post', 'enable');
	if( $enable_related_post == 'enable' && class_exists('gdlr_core_blog_style') ){
		
		$related_post_args = array(
			'blog-style' => cryptro_get_option('general', 'related-post-blog-style', 'blog-column-with-frame'), // grid-with-frame
			'blog-column-style' => cryptro_get_option('general', 'related-post-blog-column-style', 'style-2'), // grid-with-frame
			'thumbnail-size' => cryptro_get_option('general', 'related-post-thumbnail-size', 'full'),
			'column-size' => cryptro_get_option('general', 'related-post-column-size', '20'),
			'num-fetch' => cryptro_get_option('general', 'related-post-num-fetch', '3'),
			'layout' => 'fitrows',
			'excerpt' => 'specify-number',
			'excerpt-number' => cryptro_get_option('general', 'related-post-excerpt-number', '0'),
			'meta-option' => cryptro_get_option('general', 'related-post-meta-option', array()),
		...
		if( $query->have_posts() ){
			...
			echo '<div class="cryptro-single-related-post-wrap" >';
			echo '<div class="cryptro-single-related-post-container gdlr-core-container" >';
			echo '<div class="cryptro-single-related-post-content" >';
			echo '<h3 class="cryptro-single-related-post-title cryptro-item-pdlr" >' . esc_html__('Related Posts', 'cryptro') . '</h3>';
			...
			echo '</div>'; // cryptro-single-related-post-content
			echo '</div>'; // cryptro-single-related-post-container
			echo '</div>'; // cryptro-single-related-post-wrap
	...
	if( comments_open() || get_comments_number() ){
		echo '<div class="cryptro-comment-wrapper">';
		echo '<div class="cryptro-comment-container cryptro-container">';
		echo '<div class="cryptro-comment-content cryptro-item-pdlr">';
		comments_template();
		echo '</div>';
		echo '</div>';
		echo '</div>';
