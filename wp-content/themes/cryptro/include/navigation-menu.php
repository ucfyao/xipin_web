<?php
	...
	*/
	...
	// custom menu
	if( !function_exists('cryptro_get_custom_menu') ){
		function cryptro_get_custom_menu( $settings = array() ){
			if( !empty($settings['type']) ){
				if( $settings['type'] == 'overlay' ){
					cryptro_get_overlay_menu($settings);
				}else if( $settings['type'] == 'left' || $settings['type'] == 'right' ){
					$settings['slide'] = $settings['type'];
					cryptro_get_mmenu($settings);
				}
			}
		}
	}
	...
	// menu icon
	if( !function_exists('cryptro_get_mobile_menu_icon') ){
		function cryptro_get_mobile_menu_icon( $settings = array() ){

			$settings = wp_parse_args($settings, array(
				'href' => '#',
				'button-type' => cryptro_get_option('general', 'right-menu-style', 'hamburger-with-border'),
 				'button-class' => '',
				'icon-class' => 'icon_menu'
			));

			$button_class  = $settings['button-class'];
			$button_class .= ' cryptro-mobile-button-' . $settings['button-type'];
	...
	// overlay menu
	if( !function_exists('cryptro_get_overlay_menu') ){
		function cryptro_get_overlay_menu( $settings = array() ){
			...
			echo '<div class="cryptro-overlay-menu ' . esc_attr($settings['container-class']) . '" id="' . esc_attr($settings['id']) . '" >';
			
			$settings['button-class'] = 'cryptro-overlay-menu-icon ' . $settings['button-class'];
			cryptro_get_mobile_menu_icon($settings);

			echo '<div class="cryptro-overlay-menu-content cryptro-navigation-font" >';
			echo '<div class="cryptro-overlay-menu-close" ></div>';

			echo '<div class="cryptro-overlay-menu-row" >';
			echo '<div class="cryptro-overlay-menu-cell" >';
			wp_nav_menu(array(
	...
	// mmenu
	if( !function_exists('cryptro_get_mmenu') ){
		function cryptro_get_mmenu( $settings = array() ){
			...
			$settings['button-class'] = 'cryptro-mm-menu-button ' . $settings['button-class'];
			$settings['href'] = '#' .  $settings['id'];
			cryptro_get_mobile_menu_icon($settings);

			echo '<div class="cryptro-mm-menu-wrap cryptro-navigation-font" id="' . esc_attr($settings['id']) . '" data-slide="' . esc_attr($settings['slide']) . '" >';
			wp_nav_menu(array(
				'theme_location'=>$settings['theme-location'], 
				'container'=> '', 
				'menu_class'=> 'm-menu'
	...
	if( class_exists('gdlr_core_edit_nav_menu') ){
		new gdlr_core_edit_nav_menu(array(
			'icon-class' => array(
				'title' => esc_html__('Menu Icon Class', 'cryptro'),
				'type' => 'text',
			),
			'enable-mega-menu' => array(
				'title' => esc_html__('Enable Mega Menu', 'cryptro'),
				'type' => 'checkbox',
				'depth' => '0'
			),
			'mega-menu-width' => array(
				'title' => esc_html__('Mega Menu Width ( Fill value with % or px )', 'cryptro'),
				'type' => 'text',
				'default' => '100%',
				'depth' => '0'
			),
			'hide-menu-title' => array(
				'title' => esc_html__('Hide Menu Title', 'cryptro'),
				'type' => 'checkbox',
				'depth' => '1'
			),
			'mega-menu-section-size' => array(
				'title' => esc_html__('Section Size ( Only for mega menu )', 'cryptro'),
				'type' => 'combobox',
				'options' => array( 
			...
			'mega-menu-section-content' => array(
				'title' => esc_html__('Section Content ( Only for mega menu )', 'cryptro'),
				'type' => 'textarea',
				'depth' => '1'
			),
		));
	...
	if( !class_exists('cryptro_menu_walker') ){
		
		// from wp-includes/nav-menu-template.php file
		class cryptro_menu_walker extends Walker_Nav_Menu{
			...
			function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
				...
				if( $depth == 0 ){
					...
					if( ceil($this->top_level_items / 2) + 1 == $this->top_level_count ){
						$center_nav_item = apply_filters('cryptro_center_menu_item', '');
						if( !empty($center_nav_item) ){
							$output .= '<li class="cryptro-center-nav-menu-item" >' . $center_nav_item . '</li>';
						}
					}
				}
				...
				if( $depth == 0 ){
					if( $item->gdlr_core_nav_menu_custom['enable-mega-menu'] == 'disable' ){
						$class_names .= ' cryptro-normal-menu';
					}else{
						$class_names .= ' cryptro-mega-menu';
					}
				}else if( $depth == 1 ){
				...
				if( $depth == 1 && !empty($item->gdlr_core_nav_menu_custom['mega-menu-section-content']) ){
					$item_output .= '<div class="cryptro-mega-menu-section-content">';
